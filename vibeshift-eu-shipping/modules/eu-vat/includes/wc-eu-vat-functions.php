...
function wc_eu_vat_maybe_add_zero_tax_display( $total_rows, $order, $tax_display ) {
	...
	if ( wc_tax_enabled() && $is_vat_exempt && $is_valid && empty( $order->get_tax_totals() ) ) {
		 ...
		 * @since 2.8.1
		 */
		$zero_tax_reason    = apply_filters( 'wc_eu_vat_number_zero_tax_reason', __( 'Supply of services subject to reverse charge', 'vibeshift-eu-shipping' ) );
		...
		if ( 'excl' === $tax_display ) {
			...
			$append_zero_tax = sprintf( esc_html__( ' (inc. 0.00%% %1$s) ', 'vibeshift-eu-shipping' ), WC()->countries->tax_or_vat() ) . $display_tax_reason;
	...
	if ( true === $is_valid ) {
		$order->add_order_note(
			sprintf(
				...
				__( 'VAT number %s was validated successfully.', 'vibeshift-eu-shipping' ),
				$vat_number
			)
		);
		return;
