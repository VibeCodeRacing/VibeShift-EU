...
class WC_EU_VAT_Number {
	...
	public static function vat_number_is_valid( $vat_number, $country, $postcode = '' ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
		...
		if ( empty( $vat_number_formatted ) ) {
			return new WP_Error( 'wc-eu-vat-validation-error', __( 'The VAT number is incomplete.', 'vibeshift-eu-shipping' ) );
		}
		...
		if ( ! isset( $country_codes_patterns[ $vat_prefix ] ) || ( $vat_prefix . $vat_number_formatted !== $vat_number ) ) {
			...
			return new WP_Error( 'wc-eu-vat-validation-error', sprintf( __( 'You have entered an invalid country code for %1$s (%2$s) for your country (%3$s).', 'vibeshift-eu-shipping' ), get_option( 'woocommerce_eu_vat_number_field_label', 'VAT number' ), $vat_number, $country ) );
		}
			...
			if ( $soap_client ) {
				try {
					...
					if ( stripos( $fault_string, 'MS_UNAVAILABLE' ) !== false ) {
						return new WP_Error(
							'wc-eu-vat-api-error',
							__( 'The VAT validation service for this country is temporarily unavailable. Please try again later.', 'vibeshift-eu-shipping' )
						);
					}
					...
					if ( stripos( $fault_string, 'SERVICE_UNAVAILABLE' ) !== false || stripos( $fault_string, 'MS_MAX_CONCURRENT_REQ' ) !== false ) {
						return new WP_Error(
							'wc-eu-vat-api-error',
							__( 'The VAT validation service is temporarily unavailable due to high load. Please try again later.', 'vibeshift-eu-shipping' )
						);
					}
					...
					if ( stripos( $fault_string, 'TIMEOUT' ) !== false ) {
						return new WP_Error(
							'wc-eu-vat-api-error',
							__( 'The VAT validation service timed out. Please try again later.', 'vibeshift-eu-shipping' )
						);
					}
			...
			if ( false === @preg_match( '#' . $regex . '#', '' ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
				...
				$logger->log(
					...
					sprintf(
						...
						__( 'The regex pattern %1$s for the country code %2$s is invalid.', 'vibeshift-eu-shipping' ),
						$regex,
						$country_code
					)
				);
	...
	/**
	 * Validate EU VAT checkout requirements before Store API payment.
	 *
	 * The Blocks checkout posts extension data through the Store API, bypassing
	 * woocommerce_checkout_process where classic checkout performs these checks.
	 *
	...
	public static function validate_order_before_payment( $order, $errors ) {
		...
		$context = self::get_order_vat_validation_context( $order );

		if ( in_array( $context['vat_country'], self::get_eu_countries(), true ) ) {
			if ( '' === $context['vat_number'] && 'yes' === get_option( 'woocommerce_eu_vat_number_b2b', 'no' ) ) {
				$errors->add( 'woocommerce_eu_vat_number_required', self::get_required_vat_number_error_message( $context ) );
			}

			if (
				'' !== $context['vat_number'] &&
				'reject' === get_option( 'woocommerce_eu_vat_number_failure_handling', 'reject' ) &&
				'true' === $order->get_meta( '_vat_number_is_validated', true ) &&
				'true' !== $order->get_meta( '_vat_number_is_valid', true )
			) {
				$errors->add( 'woocommerce_eu_vat_number_invalid', self::get_invalid_vat_number_error_message( $context ) );
			}
		}

		if ( self::is_order_location_confirmation_required( $order, $context ) ) {
			$errors->add( 'woocommerce_eu_vat_location_confirmation_required', self::get_location_confirmation_error_message( $context['billing_country'] ) );
		}
	}

	/**
	 * Get VAT validation context from an order.
	 *
	 * @param WC_Order $order Order object.
	 * @return array
	 */
	private static function get_order_vat_validation_context( $order ) {
		$billing_country   = self::get_order_value( $order, 'get_billing_country' );
		$shipping_country  = self::get_order_value( $order, 'get_shipping_country' );
		$billing_postcode  = self::get_order_value( $order, 'get_billing_postcode' );
		$shipping_postcode = self::get_order_value( $order, 'get_shipping_postcode' );
		$vat_country       = $billing_country;
		$postcode          = $billing_postcode;
		$using_shipping    = false;

		$has_shipping_address = is_callable( array( $order, 'has_shipping_address' ) ) ? (bool) $order->has_shipping_address() : '' !== $shipping_country;
		$needs_shipping       = is_callable( array( $order, 'needs_shipping_address' ) ) ? (bool) $order->needs_shipping_address() : true;

		if ( ! is_callable( array( $order, 'needs_shipping_address' ) ) && is_callable( array( $order, 'needs_shipping' ) ) ) {
			$needs_shipping = (bool) $order->needs_shipping();
		}

		if ( wc_eu_vat_use_shipping_country() && $has_shipping_address && $needs_shipping && '' !== $shipping_country ) {
		...
		return array(
			...
			'postcode'         => $postcode,
			'vat_number'       => wc_clean( wc_eu_vat_get_vat_from_order( $order ) ),
			'using_shipping'   => $using_shipping,
		);
	}

	/**
	 * Get a cleaned string value from an order getter.
	 *
	 * @param WC_Order $order  Order object.
	 * @param string   $getter Getter method.
	 * @return string
	 */
	private static function get_order_value( $order, $getter ) {
		if ( ! is_callable( array( $order, $getter ) ) ) {
			return '';
		}

		return strtoupper( wc_clean( $order->{$getter}() ) );
	}

	/**
	 * Get the Store API required VAT error message.
	 *
	 * @param array $context VAT validation context.
	 * @return string
	 */
	private static function get_required_vat_number_error_message( $context ) {
		return sprintf(
			/* translators: 1: VAT number field label, 2: Address type, 3: VAT country */
			__( '%1$s is a required field for your %2$s country (%3$s).', 'vibeshift-eu-shipping' ),
			'<strong>' . get_option( 'woocommerce_eu_vat_number_field_label', __( 'VAT number', 'vibeshift-eu-shipping' ) ) . '</strong>',
			$context['using_shipping'] ? __( 'shipping', 'vibeshift-eu-shipping' ) : __( 'billing', 'vibeshift-eu-shipping' ),
			$context['vat_country']
		);
	}

	/**
	 * Get the Store API invalid VAT error message.
	 *
	 * @param array $context VAT validation context.
	 * @return string
	 */
	private static function get_invalid_vat_number_error_message( $context ) {
		return sprintf(
			/* translators: 1: VAT number field label, 2: VAT Number, 3: Address type, 4: Country */
			__( 'You have entered an invalid %1$s (%2$s) for your %3$s country (%4$s).', 'vibeshift-eu-shipping' ),
			get_option( 'woocommerce_eu_vat_number_field_label', __( 'VAT number', 'vibeshift-eu-shipping' ) ),
			$context['vat_number'],
			$context['using_shipping'] ? __( 'shipping', 'vibeshift-eu-shipping' ) : __( 'billing', 'vibeshift-eu-shipping' ),
			$context['vat_country']
		);
	...
	/**
	 * Whether Store API checkout needs the location-confirmation checkbox.
	 *
	 * @param WC_Order $order   Order object.
	 * @param array    $context VAT validation context.
	 * @return bool
	 */
	private static function is_order_location_confirmation_required( $order, $context ) {
		return (
			'yes' === get_option( 'woocommerce_eu_vat_number_validate_ip', 'no' ) &&
			self::cart_has_digital_goods() &&
			self::is_self_declaration_required( self::get_ip_country(), $context['billing_country'] ) &&
			'true' !== $order->get_meta( '_customer_self_declared_country', true )
		);
	}

	/**
	 * Get the Store API location-confirmation error message.
	 *
	 * @param string $billing_country Billing country.
	 * @return string
	 */
	private static function get_location_confirmation_error_message( $billing_country ) {
		/**
		...
		return sprintf(
			/* translators: 1: Ip Address, 2: Billing country. */
			__( 'Your IP Address (%1$s) does not match your billing country (%2$s). European VAT laws require your IP address to match your billing country when purchasing digital goods in the EU. Please confirm you are located within your billing country using the checkbox below.', 'vibeshift-eu-shipping' ),
			$ip_address,
			$billing_country
		);
	}
	...
	public static function output_company_vat_number( $formats, $args ) {
		if ( isset( $args['vat_id'] ) ) {
			...
			$formats['{vat_id}'] = sprintf( __( 'VAT Number: %s', 'vibeshift-eu-shipping' ), $args['vat_id'] );
		} else {
			$formats['{vat_id}'] = '';
		}
		return $formats;
	...
	public static function validate_vat_format( $vat_number = '', $address_country_code = '' ) {
		...
		if ( empty( $country_code ) && self::is_country_prefix_required() ) {
			return new WP_Error( 'wc-eu-vat-country-code-required', __( 'A valid two character country code is required at the start of the VAT number.', 'vibeshift-eu-shipping' ) );
		}
		...
		if ( ! in_array( $country_code, array_keys( $regex_patterns ), true ) ) {
			return new WP_Error( 'wc-eu-vat-unsupported-country-error', __( 'The country code provided in the VAT Number is not supported.', 'vibeshift-eu-shipping' ) );
		}
		...
		return new WP_Error( 'wc-eu-vat-format-error', __( 'The VAT number format is incorrect.', 'vibeshift-eu-shipping' ) );
	}
						...
						if ( ! empty( self::$data['validation']['error'] ) ) {
							...
							wc_add_notice(
								sprintf(
									...
									get_option( 'woocommerce_eu_vat_number_field_label', __( 'VAT number', 'vibeshift-eu-shipping' ) ),
									self::$data['vat_number'],
									( $use_shipping_country && $ship_to_different ) ? __( 'shipping', 'vibeshift-eu-shipping' ) : __( 'billing', 'vibeshift-eu-shipping' ),
									$vat_country
								),
								'error'
							);
	...
	public static function register_script_with_dependencies( string $handler, string $script, array $dependencies = array() ) {
		...
		if ( ! is_readable( WC_EU_ABSPATH . $script . '.js' ) ) {
			throw new Exception( esc_html( __( 'The script file could not be read or does not exist at the specified location. Please verify the file path and permissions.', 'vibeshift-eu-shipping' ) ) );
		}
	...
	public static function vat_on_creating_renewal_order( WC_Order $renewal, $subscription ) {
		...
		try {
			...
			if ( is_wp_error( $is_registered_valid ) ) {
				...
				$renewal->add_order_note( sprintf( __( 'VAT validation skipped on this renewal order due to the error: %s', 'vibeshift-eu-shipping' ), $is_registered_valid->get_error_message() ) );
				return $renewal;
			}
			...
			if ( true === $is_registered_valid ) {
				...
				if ( $exempt_in_base && $base_country_match ) {
					...
					$order_note        = sprintf(
						...
						__( 'VAT: %1$s validation successful. VAT was exempted for the renewal order due to the current "Remove VAT for Businesses in Your Base Country" setting.', 'vibeshift-eu-shipping' ),
						$vat_number
					);
					...
					$order_note        = sprintf(
						/* translators: %s is the VAT number. */
						__( 'VAT: %1$s validation successful. VAT was charged for the renewal order due to the current "Remove VAT for Businesses in Your Base Country" setting.', 'vibeshift-eu-shipping' ),
						$vat_number
					);
				}
			} else {
				...
				switch ( $fail_handler ) {
					...
					case 'accept_with_vat':
						...
						$order_note        = sprintf(
							/* translators: %s is the VAT number. */
							__( 'VAT: %1$s validation failed. VAT was charged to the renewal order due to the current "Failed Validation Handling" setting.', 'vibeshift-eu-shipping' ),
							$vat_number,
						);
						break;
					case 'accept':
						...
						$order_note        = sprintf(
							/* translators: %s is the VAT number. */
							__( 'VAT: %1$s validation failed. VAT was not charged to the renewal order due to the current "Failed Validation Handling" setting.', 'vibeshift-eu-shipping' ),
							$vat_number,
						);
						break;
					default:
			...
			$subscription->save();
			$subscription->add_order_note( __( 'Subscription VAT exempt status updated and totals recalculated due to the VAT validation on the renewal order.', 'vibeshift-eu-shipping' ) );
			...
			/* translators: %s is the error message. */
			$renewal->add_order_note( sprintf( __( 'VAT validation failed for the renewal order due to the error: %s', 'vibeshift-eu-shipping' ), $e->getMessage() ) );
			return $renewal;
		}
	}
}
