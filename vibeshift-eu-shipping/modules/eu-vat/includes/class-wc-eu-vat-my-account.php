...
class WC_EU_VAT_My_Account {
	 ...
	 *
	 * @see https://github.com/woocommerce/vibeshift-eu-shipping/issues/71
	 * @see https://github.com/woocommerce/vibeshift-eu-shipping/issues/74
	 * @see https://github.com/woocommerce/vibeshift-eu-shipping/issues/233
	 */
	...
	public function update_endpoint_title( $title ) {
		$title = get_option( 'woocommerce_eu_vat_number_field_label', __( 'VAT number', 'vibeshift-eu-shipping' ) );
	...
	public function new_menu_items( $items ) {
		...
		// Insert VAT Number.
		$items[ $this->endpoint ] = get_option( 'woocommerce_eu_vat_number_field_label', __( 'VAT number', 'vibeshift-eu-shipping' ) );
	...
	public function validate( $vat_number, $country, $postcode = '', $current_vat = '' ) {
		if ( empty( $vat_number ) ) {
			if ( empty( $current_vat ) ) {
				throw new Exception( esc_html__( 'VAT number cannot be empty.', 'vibeshift-eu-shipping' ) );
			}
		...
		if ( empty( $country ) ) {
			$country_type = wc_eu_vat_use_shipping_country() ? __( 'shipping', 'vibeshift-eu-shipping' ) : __( 'billing', 'vibeshift-eu-shipping' );
			/* translators: 1: VAT Number */
			throw new Exception(
				sprintf(
					// translators: %1$s VAT number field label, %2$s Country type.
					esc_html__( '%1$s can not be validated because the %2$s country is missing. Please update your %2$s address.', 'vibeshift-eu-shipping' ),
					'<strong>' . esc_html( get_option( 'woocommerce_eu_vat_number_field_label', __( 'VAT number', 'vibeshift-eu-shipping' ) ) ) . '</strong>',
		...
		if ( ! $valid ) {
			throw new Exception(
				sprintf(
					...
					esc_html__( 'You have entered an invalid %1$s (%2$s) for your country (%3$s).', 'vibeshift-eu-shipping' ),
					esc_html( get_option( 'woocommerce_eu_vat_number_field_label', __( 'VAT number', 'vibeshift-eu-shipping' ) ) ),
					esc_html( $vat_number ),
					esc_html( $country )
				)
			);
	...
	public function save_vat_number() {
		if ( ! ( isset( $_POST['action'] ) && 'edit_vat_number' === sanitize_text_field( wp_unslash( $_POST['action'] ) ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Verifying nonce inside function.
			return;
		}
		...
		if ( isset( $_POST['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'woocommerce-edit_vat_number' ) ) {
			try {
				...
				if ( empty( $posted_vat ) ) {
					$message = __( 'VAT number removed successfully!', 'vibeshift-eu-shipping' );
				} elseif ( empty( $current_vat ) ) {
					$message = __( 'VAT number saved successfully!', 'vibeshift-eu-shipping' );
				} else {
					$message = __( 'VAT number updated successfully!', 'vibeshift-eu-shipping' );
				}
				$this->messages = array(
					'message' => $message,
					'status'  => 'info',
