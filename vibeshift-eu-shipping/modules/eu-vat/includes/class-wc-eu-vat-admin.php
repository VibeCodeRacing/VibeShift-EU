...
class WC_EU_VAT_Admin {
	...
	public static function add_vat_number_field( $fields ) {
		...
		$fields['vat_number'] = array(
			'label' => get_option( 'woocommerce_eu_vat_number_field_label', __( 'VAT number', 'vibeshift-eu-shipping' ) ),
			'show'  => false,
	...
	public static function add_meta_boxes() {
		...
		add_meta_box( 'wc_eu_vat', __( 'EU VAT', 'vibeshift-eu-shipping' ), array( __CLASS__, 'output' ), $screen, 'side' );
	}
	...
	public static function styles_and_scripts( $hook ) {
		...
		// Read-only screen detection to decide whether to enqueue order-edit assets; no state is changed, so nonce verification is not applicable.
		if ( 'woocommerce_page_wc-orders' === $hook && isset( $_GET['id'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$is_order_edit_screen = true;
		} else if ( 'woocommerce_page_wc-orders' === $hook && isset( $_GET['action'] ) && 'new' === sanitize_text_field( wp_unslash( $_GET['action'] ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$is_order_edit_screen = true;
	...
	public static function output() {
		...
		<table class="wc-eu-vat-table" cellspacing="0">
			<tbody>
				<tr>
					<th><?php esc_html_e( 'B2B', 'vibeshift-eu-shipping' ); ?></th>
					<td><?php echo $data->vat_number ? esc_html__( 'Yes', 'vibeshift-eu-shipping' ) : esc_html__( 'No', 'vibeshift-eu-shipping' ); ?></td>
					<td></td>
				</tr>

				<?php if ( $data->vat_number ) : ?>
					<tr>
						<th><?php echo esc_html( get_option( 'woocommerce_eu_vat_number_field_label', __( 'VAT number', 'vibeshift-eu-shipping' ) ) ); ?></th>
						<td><?php echo esc_html( $data->vat_number ); ?></td>
						<td>
							<?php
							if ( ! $data->validated ) {
								echo '<span class="tips" data-tip="' . wc_sanitize_tooltip( __( 'Validation was not possible', 'vibeshift-eu-shipping' ) ) . '">?<span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							} else {
								echo $data->valid ? '&#10004;' : '&#10008;';
							}
							?>
				...
				<?php else : ?>
					<tr>
						<th><?php esc_html_e( 'IP Address', 'vibeshift-eu-shipping' ); ?></th>
						<td><?php echo $data->ip_address ? esc_html( $data->ip_address ) : esc_html__( 'Unknown', 'vibeshift-eu-shipping' ); ?></td>
						<td></td>
					</tr>
					<tr>
						<th><?php esc_html_e( 'IP Country', 'vibeshift-eu-shipping' ); ?></th>
						<td>
							<?php
							if ( $data->ip_country ) {
								...
								if ( $data->billing_country === $data->ip_country ) {
									...
									printf(
										'(%s %s)',
										esc_html__( 'User self-declared country as', 'vibeshift-eu-shipping' ),
								...
								esc_html_e( 'Unknown', 'vibeshift-eu-shipping' );
							}
							?>
						</td>
						<td></td>
					...
					<tr>
						<th><?php esc_html_e( 'Billing Country', 'vibeshift-eu-shipping' ); ?></th>
						<td><?php echo $data->billing_country ? esc_html( $countries[ $data->billing_country ] ) : esc_html__( 'Unknown', 'vibeshift-eu-shipping' ); ?></td>
						<td></td>
					</tr>
				<?php endif; ?>
			</tbody>
	...
	public static function add_column( $existing_columns ) {
		...
		foreach ( $existing_columns as $existing_column_key => $existing_column ) {
			...
			if ( 'shipping_address' === $existing_column_key ) {
				$columns['eu_vat'] = __( 'EU VAT', 'vibeshift-eu-shipping' );
			}
		}
			...
			if ( ! self::is_eu_order( $order ) ) {
				...
				if ( $data->vat_number ) {
					...
					if ( $data->billing_country === $data->ip_country ) {
						...
						esc_html_e( '(self-declared)', 'vibeshift-eu-shipping' );
					} else {
						echo '<span style="color:red">&#10008;</span>';
					}
				}
	...
	public static function admin_order( $args, $order ) {
		 ...
		 */
		$billing_country  = isset( $_POST['_billing_country'] ) ? sanitize_text_field( wp_unslash( $_POST['_billing_country'] ) ) : $order->get_billing_country(); // phpcs:ignore WordPress.Security.NonceVerification.Missing
		$shipping_country = isset( $_POST['_shipping_country'] ) ? sanitize_text_field( wp_unslash( $_POST['_shipping_country'] ) ) : $order->get_shipping_country(); // phpcs:ignore WordPress.Security.NonceVerification.Missing
		...
		$billing_postcode  = isset( $_POST['_billing_postcode'] ) ? sanitize_text_field( wp_unslash( $_POST['_billing_postcode'] ) ) : $order->get_billing_postcode(); // phpcs:ignore WordPress.Security.NonceVerification.Missing
		$shipping_postcode = isset( $_POST['_shipping_postcode'] ) ? sanitize_text_field( wp_unslash( $_POST['_shipping_postcode'] ) ) : $order->get_shipping_postcode(); // phpcs:ignore WordPress.Security.NonceVerification.Missing
		 ...
		 * found, get it from the order (editing the order).
		 */
		$vat_number = isset( $_POST['_billing_vat_number'] ) ? sanitize_text_field( wp_unslash( $_POST['_billing_vat_number'] ) ) : wc_eu_vat_get_vat_from_order( $order ); // phpcs:ignore WordPress.Security.NonceVerification.Missing

		$country  = $billing_country;
		$postcode = $billing_postcode;
		$type     = __( 'billing', 'vibeshift-eu-shipping' );
		if ( $order->has_shipping_address() && wc_eu_vat_use_shipping_country() ) {
			$country  = $shipping_country;
			$postcode = $shipping_postcode;
			$type     = __( 'shipping', 'vibeshift-eu-shipping' );
		}
		...
		try {
			...
			if ( ! $valid ) {
				...
				throw new Exception( sprintf( esc_html__( 'You have entered an invalid %1$s (%2$s) for your %4$s country (%3$s).', 'vibeshift-eu-shipping' ), get_option( 'woocommerce_eu_vat_number_field_label', __( 'VAT number', 'vibeshift-eu-shipping' ) ), $vat_number, $country, $type ) );
			}
		} catch ( Exception $e ) {
			$order->update_meta_data( '_vat_number_is_valid', 'false' );
	...
	public static function add_customer_meta_fields( $fields ) {
		if ( wc_eu_vat_use_shipping_country() ) {
			$fields['shipping']['fields']['vat_number'] = array(
				'label'       => esc_html__( 'VAT number', 'vibeshift-eu-shipping' ),
				'description' => '',
			);
		...
		<div class="notice notice-info is-dismissible">
			...
			if ( 'GB' === $base_country && preg_match( '/^(bt).*$/i', $base_postcode ) ) {
				...
				<p>
					...
					echo sprintf( esc_html__( 'Based on your store address, we\'ve identified your store location as %1$sNorthern Ireland%2$s, where European Union VAT laws apply. These laws require all goods to be %1$sshipped directly from Northern Ireland and not any other country%2$s. %3$s The %1$sVibeShift EU Shipping%2$s plugin assumes that the store abides by these laws and all shipments originate from %1$sNorthern Ireland%2$s.', 'vibeshift-eu-shipping' ), '<strong>', '</strong>', '<br/>' );
					?>
				</p>
				<hr>
				<?php
			...
			<p>
				...
				echo sprintf( esc_html__( 'By using %1$sVibeShift EU Shipping%2$s plugin, you\'ve agreed that the use of this plugin cannot be considered as tax advice. We recommend consulting a local tax professional for tax compliance or if you have any tax specific questions.', 'vibeshift-eu-shipping' ), '<strong>', '</strong>' );
				?>
			</p>

			<p>
				<a href="<?php echo esc_url( $dismiss_url ); ?>" class="button button-primary"><?php echo esc_html__( 'I understand', 'vibeshift-eu-shipping' ); ?></a>
			</p>
		</div>
		<?php
	}
	...
	public static function dismiss_eu_vat_disclaimer() {
		...
		$maybe_dismiss = sanitize_text_field( wp_unslash( $_GET['dismiss_eu_vat_disclaimer'] ) );
			...
			<p>
				...
				echo wp_kses_post(
					sprintf(
						...
						esc_html__( '%1$sVibeShift EU Shipping%2$s plugin adds a VAT number field to the checkout block though it may appear after the "Place Order" button. Please %3$sedit%4$s the checkout block to adjust the position of the VAT Number field accordingly.', 'vibeshift-eu-shipping' ),
						'<strong>',
						'</strong>',
	...
	public static function get_vat_number_field_label() {
		...
		if ( empty( $label ) ) {
			return _x( 'VAT number', 'Default Field Label', 'vibeshift-eu-shipping' );
		}
